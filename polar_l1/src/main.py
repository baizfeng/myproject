...
def main(primaryFile, geoFile, resultPath, auxPath):
    ...
    output_template = "{rootpath}/{satellite}/{sensor}/{level}/{yyyy}/{yyyymmdd}/{sgs}_{satellite}_{sensor}_GBAL_{level}_{resolution}_GLL_{yyyymmdd}_{HHMMSS}.{format}"
    ...
    # 保存TIFF
    save_tiff(
        tiff_data, lon, lat, names["TIFF"], resolution=0.005, metadata=tiff_metadata
    )
    ...
    save_netcdf(
        ...
        lat,
        names["NC"],
        var_attrs=nc_var_attrs,
        global_attrs=nc_global_attrs,
    )
    ...
    draw.draw_single_map(
        names["TIFF"], names["PNG.IR11"], template, ir11qml, mapinfos["IR11"]
    )
    log("IR11产品专题图输出完成:" + names["PNG.IR11"])

    draw.draw_single_map(
        names["TIFF"], names["PNG.IR12"], template, ir12qml, mapinfos["IR12"]
    )
